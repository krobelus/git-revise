...
def dfs(fixups: Dict[Oid, List[Oid]], visited: Set[Oid], node: Oid) -> int:
    if node in visited:
        raise CyclicFixupError(f"fixups would create cycle in {node}")
    visited.add(node)
    return 1 + sum(dfs(fixups, visited, fixup) for fixup in fixups.get(node, []))
        ...
        for idx, target in enumerate(new_todos):
            if target.commit.summary().startswith(
                ...
                number_of_transitive_fixup_commits = (
                    dfs(fixups, set(), target.commit.oid) - 1
                )
                break
